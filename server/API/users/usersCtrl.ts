    ...
    connection.query(query, (error, results: any, fields) => {
      if (error) {
        return res.status(500).send({
          success: false,
          error:
            "Failed to insert user data into database. Check your details. Perhaps you are trying to enter already registered data.",
        });
      }
      ...
      if (!secret)
        return res
          ...
          .send({
            success: false,
            error: "Couldn't load secret code from .env",
          });
  ...
  try {
    ...
    connection.query(query, async (err, results: RowDataPacket[], fields) => {
      try {
        ...
        if (!Array.isArray(results) || results.length === 0) {
          throw new Error(
            "Email or password doesn't match or user doesn't exist."
          );
        }
        const isMatch = await bcrypt.compare(password, results[0].password);
    ...
    connection.query(query, (err, result: ResultSetHeader) => {
      if (err) {
        return res
          ...
          .json({
            error:
              "Something went wrong. Error deleting user from the database.",
          });
      }
...
export async function getUserByID(req: express.Request, res: express.Response) {
  try {
    ...
    connection.query(query, (err, result: ResultSetHeader) => {
      ...
      return res
        .status(200)
        .json({ success: "The user has been dedicated.", user: result });
    });
  } catch (error: any) {
    res.status(500).send({ success: false, error: error.message });
  }
